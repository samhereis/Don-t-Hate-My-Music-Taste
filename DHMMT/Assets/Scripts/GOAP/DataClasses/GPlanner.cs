...
namespace GOAP.GoapDataClasses
{
    ...
    {
        public Queue<GAction> Plan(List<GAction> actions, Dictionary<GOAPStrings, int> goal, GoapStates beliefStates)
        {
        ...
        private bool BuildGraph(Node parent, List<Node> leaves, List<GAction> usableActions, Dictionary<GOAPStrings, int> goal)
        {
            ...
            foreach (GAction action in usableActions)
            {
                ...
                if (action.IsAhievableGiven(parent.state))
                {
                    ...
                    Dictionary<GOAPStrings, int> currentState = new Dictionary<GOAPStrings, int>(parent.state);
                    ...
                    foreach (KeyValuePair<GOAPStrings, int> eff in action.baseSettings.afterEffects)
                    {
        ...
        private bool GoalAchieved(Dictionary<GOAPStrings, int> goal, Dictionary<GOAPStrings, int> state)
        {
            foreach (KeyValuePair<GOAPStrings, int> g in goal)
            {
