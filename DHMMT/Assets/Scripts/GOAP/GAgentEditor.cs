    ...
    public class GAgentVisualEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            foreach (GAction a in agent.gameObject.GetComponent<GAgent>().baseSettings.actions)
            {
                ...
                foreach (KeyValuePair<GOAPStrings, int> p in a.baseSettings.preConditions)
                    pre += p.Key + ", ";
                foreach (KeyValuePair<GOAPStrings, int> e in a.baseSettings.afterEffects)
                    eff += e.Key + ", ";
            ...
            {
                foreach (KeyValuePair<GOAPStrings, int> sg in g.Key.subGoals)
                    GUILayout.Label("=====  " + sg.Key);
            }
            GUILayout.Label("Beliefs: ");
            foreach (KeyValuePair<GOAPStrings, int> sg in agent.gameObject.GetComponent<GAgent>().baseSettings.localStates.GetStates())
            {
                GUILayout.Label("=====  " + sg.Key);
            }
